...
def write_file(file_path: str, content: str) -> str:
    ...
    try:
        ...
        return success_response(f"Successfully wrote to {file_path}", file_path=file_path, bytes_written=len(content))
    except Exception as e:
...
def delete_file(file_path: str) -> str:
    ...
    try:
        ...
        full_path.unlink()
        return success_response(f"Successfully deleted {file_path}", file_path=file_path)
    except Exception as e:
    ...
    try:
        ...
        for todo in todos:
            if not all(key in todo for key in ["id", "content", "status", "priority"]):
                return error_response("Each todo must have id, content, status, and priority")
