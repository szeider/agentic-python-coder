...
class UVKernelManager(KernelManager):
    ...
    def __init__(self, with_packages: Optional[List[str]] = None, cwd: Optional[str] = None, **kwargs):
...
class PythonKernel:
    ...
    def __init__(self, cwd: Optional[str] = None, with_packages: Optional[List[str]] = None):
        ...
        try:
            ...
            self.kc.wait_for_ready(timeout=30)  # Increased timeout for UV package installation
...
def get_kernel(cwd: Optional[str] = None, with_packages: Optional[List[str]] = None) -> PythonKernel:
    ...
    with _kernel_lock:
        ...
        if _kernel is None:
            ...
            logger.info(f"Working directory changed from {_kernel.cwd} to {cwd}, restarting kernel")
            kernel_is_stale = True
        elif set(_kernel.with_packages) != set(requested_packages):
            logger.info(f"Package list changed from {_kernel.with_packages} to {requested_packages}, restarting kernel")
            kernel_is_stale = True
        ...
        if kernel_is_stale:
            ...
            logger.info(f"Creating new kernel with cwd={cwd}, packages={requested_packages}")
            try:
...
def format_output(output: Dict[str, str]) -> str:
    ...
    result = {
        ...
        "result": output.get('result') if output.get('result') and output['result'] != 'None' else None,
