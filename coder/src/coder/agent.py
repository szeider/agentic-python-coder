...
def create_coding_agent(
    ...
    Args:
        ...
        task_prompt_path: Path to task-specific prompt file
    ...
    # Get LLM instance
    llm = get_openrouter_llm(model=model) if model else get_openrouter_llm()
    ...
    if system_prompt_path:
        ...
        prompts.append("You are a Python coding assistant with file and execution tools.")
    
    # Task prompt (specific instructions)
    if task_prompt_path:
        task_prompt = load_prompt(Path(task_prompt_path))
...
def run_agent(agent, user_input: str, thread_id: str = "default") -> tuple[List[Dict[str, Any]], Dict[str, Any]]:
                ...
                for msg in node_output["messages"]:
                            ...
                            if isinstance(args, dict):
                                if tool_name == "python_exec" and "code" in args:
                                    ...
                                    if "def " in code:
                                        ...
                                        # Class definition
                                        class_match = code.split("class ")[1].split("(")[0].split(":")[0] if "class " in code else ""
                                        print(f"  {tool_name}: defining class {class_match}")
                                    elif ".py" in code and "open" in code:
                                        # Running a file
                                        import re
                                        file_match = re.search(r'["\']([^"\'\\/]+\.py)["\']', code)
                                        if file_match:
                                            print(f"  {tool_name}: executing file {file_match.group(1)}")
                                        else:
                                            print(f"  {tool_name}: executing Python file")
                                        ...
                                        print(f"  {tool_name}: {code.strip()[:50]}{'...' if len(code.strip()) > 50 else ''}")
                                    ...
                                    todos = args["todos"]
                                    for todo in todos:
                                        status_symbol = "☒" if todo["status"] == "completed" else "☐" if todo["status"] == "pending" else "▶"
                    ...
                    if hasattr(msg, "response_metadata"):
                        ...
                        if usage:
                            ...
                            stats["token_consumption"]["output_tokens"] += usage.get("completion_tokens", 0)
                    ...
                    if hasattr(msg, "usage_metadata") and msg.usage_metadata:
                        stats["token_consumption"]["input_tokens"] += msg.usage_metadata.get("input_tokens", 0)
                        stats["token_consumption"]["output_tokens"] += msg.usage_metadata.get("output_tokens", 0)
                        stats["token_consumption"]["total_tokens"] += msg.usage_metadata.get("total_tokens", 0)
                    ...
                    if hasattr(msg, "type") and msg.type == "tool":
                        ...
                        tool_name = current_tools.get(tool_id, getattr(msg, 'name', 'unknown'))
                        
                        # Don't print checkmarks, just continue
                        pass
                    elif isinstance(msg, dict) and msg.get("type") == "tool":
                        tool_id = msg.get("tool_call_id")
                        tool_name = current_tools.get(tool_id, msg.get('name', 'unknown'))
